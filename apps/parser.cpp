...
void Writer::writeCPP()
{
    ...
    for(int i=0; i!=m_infos.size();++i)
    {
        ...
        booleans+="}";
        std::string inte="{";
        //outfile<<"const static std::map<Integer,std::uint16_t> i"<<std::to_string(i)<<"{";
        ...
        inte+="}";
        //outfile<<"const static std::map<String,std::string> s"<<std::to_string(i)<<"{";
    ...
    outfile<<"std::vector<std::reference_wrapper<Terminfo::Terminfo>> Terminfo::m_inf{"<<term_list<<"};\n";
    outfile.close();
}
...
int main(int argc, char* argv[])
{
    ...
    if(parser.hasUnknown())
    {
        for(std::map<std::string,Terminfo::Capability::Type>::const_iterator it=parser.getUnknown().cbegin();it!=parser.getUnknown().cend();++it)
        {
            ...
            {
                std::cout<<"boolean capability "<<it->first<<" uknown !"<<std::endl;
                break;
            }
            case Terminfo::Capability::Type::integer :
            {
                std::cout<<"integer capability "<<it->first<<" uknown !"<<std::endl;
                break;
            }
            case Terminfo::Capability::Type::string :
            {
                std::cout<<"string capability "<<it->first<<" uknown !"<<std::endl;
                break;
            }
            default:
                break;
