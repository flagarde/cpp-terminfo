...
public:
  ...
  void setTerminfos(const std::vector<Terminfo::Terminfo> &info) {
    m_infos = info;
  }
  void write() {
    writeCPP();
...
void Writer::writeCPP() {
  ...
  for (int i = 0; i != m_infos.size(); ++i) {
    if (m_infos[i].getType().name().find('+') != std::string::npos ||
        std::find(ignore.begin(), ignore.end(), m_infos[i].getType().name()) !=
            ignore.end())
      continue;
    std::ofstream outfile(m_cpp_path + "terminals/" +
                          m_infos[i].getType().name() + ".cpp");
    std::ofstream header(m_cpp_path + "include/cpp-terminfo/" +
                         m_infos[i].getType().name() + ".hpp");
    outfile << "// terminal : " << m_infos[i].getType().name() << "\n";
    outfile << "#include \"cpp-terminfo/Terminfo.hpp\"\n";
    outfile << "#include \"cpp-terminfo/" << m_infos[i].getType().name()
            << ".hpp\"\n";

    std::string description = m_infos[i].getType().description();
    std::string aliases = "\"" + m_infos[i].getType().name() + "\"";
    for (std::size_t j = 0; j != m_infos[i].getType().aliases().size(); ++j) {
      aliases += ",\"" + m_infos[i].getType().aliases()[j] + "\"";
    }
    std::string booleans = "{";
    const std::set<Terminfo::Boolean> bools = m_infos[i].getBooleans();
    for (std::set<Terminfo::Boolean>::const_iterator it = bools.cbegin();
         it != bools.cend(); ++it) {
    ...
    // i"<<std::to_string(i)<<"{";
    const std::map<Terminfo::Integer, std::uint16_t> integers =
        m_infos[i].getIntegers();
    for (std::map<Terminfo::Integer, std::uint16_t>::const_iterator it =
             integers.cbegin();
         it != integers.cend(); ++it) {
    ...
    // s"<<std::to_string(i)<<"{";
    const std::map<Terminfo::String, std::string> strings =
        m_infos[i].getStrings();
    std::string str = "{";
    for (std::map<Terminfo::String, std::string>::const_iterator it =
             strings.cbegin();
         it != strings.cend(); ++it) {
      str += "{Terminfo::String(";
...
int main(int argc, char *argv[]) {
  ...
  if (parser.hasUnknown()) {
    for (std::map<std::string, Terminfo::Capability::Type>::const_iterator it =
             parser.getUnknown().cbegin();
         it != parser.getUnknown().cend(); ++it) {
      switch (it->second) {
      case Terminfo::Capability::Type::boolean: {
        std::cout << "boolean capability " << it->first << " unknown !"
                  << std::endl;
        break;
      }
      case Terminfo::Capability::Type::integer: {
        std::cout << "integer capability " << it->first << " unknown !"
                  << std::endl;
        break;
      }
      case Terminfo::Capability::Type::string: {
        std::cout << "string capability " << it->first << " unknown !"
                  << std::endl;
        break;
      }
      default:
