...
split(const std::string &s, const char &delimiter, const char &annuler = '\\') {
  if (s.empty())
    return {};
  ...
  if (pos_last != s.size() - 1)
    tokens.push_back(s.substr(pos_begin, s.size() - pos_begin));
  return tokens;
}
...
void Terminfo::Parser::parseCapabilities(std::string &line) {
  ...
  for (std::size_t i = 0; i != capabilities.size(); ++i)
    parseCapability(capabilities[i]);
  line.clear();
}

void Terminfo::Parser::parseCapability(const std::string &line) {
  if (line[0] == '.')
    return; // commented capability
  if (line[0] == 'O' && line[1] == 'T')
    return; // obsolete capability
  try {
    ...
    if (pos != std::string::npos) // is a string
    {
      ...
      if (key == "use") {
        m_need_reparse[m_infos[m_infos.size() - 1].getType().name()].push_back(
            value);
        return;
      }
    ...
    if (pos != std::string::npos) {
      m_need_delete[m_infos[m_infos.size() - 1].getType().name()].push_back(
          line.substr(0, line.size() - 1));
      return;
    }
    // is a boolean
    if (capabilities.know(line))
      m_infos[m_infos.size() - 1].addBoolean(capabilities.getBoolean(line));
    else
...
void Terminfo::Parser::resolveUses() {
  ...
  for (std::map<std::string, std::vector<std::string>>::iterator it =
           m_need_reparse.begin();
       it != m_need_reparse.end(); ++it) {
    for (std::vector<std::string>::reverse_iterator itt = it->second.rbegin();
  ...
  for (std::size_t i = 0; i != to_remove.size(); ++i)
    m_need_reparse.erase(to_remove[i]);
}
...
void Terminfo::Parser::resolveDeletes() {
  ...
  for (std::size_t i = 0; i != m_infos.size(); ++i) {
    std::map<std::string, std::vector<std::string>>::const_iterator find =
        m_need_delete.find(m_infos[i].getType().name());
