...
public:
  ...
  inline bool know(const std::string &name) const noexcept {
    return std::find(m_booleans.begin(), m_booleans.end(), name) !=
               m_booleans.end() ||
           std::find(m_integers.begin(), m_integers.end(), name) !=
               m_integers.end() ||
           std::find(m_strings.begin(), m_strings.end(), name) !=
               m_strings.end();
  }
  inline Capability::Type type(const std::string &name) {
    if (std::find(m_booleans.begin(), m_booleans.end(), name) !=
        m_booleans.end())
      return Capability::Type::boolean;
    else if (std::find(m_integers.begin(), m_integers.end(), name) !=
             m_integers.end())
      return Capability::Type::integer;
    else if (std::find(m_strings.begin(), m_strings.end(), name) !=
             m_strings.end())
